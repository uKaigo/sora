...
class Fun(commands.Cog, name="_fun_cog"):
    ...
    async def get_meme(self, soup: BeautifulSoup) -> namedtuple:
        ...
        if meme.attrs.get("data-type") in {"image", "video"}:
            ...
            url = list(list(list(meme.children)[1].children)[1].children)[0].attrs[
                "data-src"
            ]
        ...
        async with ctx.typing():
            try:
                image = await self.bot.apis.ksoft.images.random_reddit(
                    subreddit, remove_nsfw=not nsfw  # Para inverter o valor.
                )
                ...
                return await ctx.send(ctx.t("err_nof_found") + nsfw_message)
            ...
            embed.set_author(
                name=ctx.t("title", author=author, sub=sub), url=image.source
            )
